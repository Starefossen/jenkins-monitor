...
module.exports.summary = function jenkinsSummary(nodes) {
  ...
  if (offline > 0) {
    const nodes = offline === 1 ? 'node is' : 'nodes are';

    summary.push(`${offline} Jenkins ${nodes} offline`);
  }

  if (online > 0) {
    const where = offline === 0 ? ' Jenkins' : '';
    const nodes = online === 1 ? 'node is' : 'nodes are';

    summary.push(`${online}${where} ${nodes} online`);
  }
